  ...
  return (
    <div className='py-20' id='projects'>
        <h1 className='heading'>
            See it for {''}
            <span className='text-purple'>yourself</span>
        </h1>
        <div className='flex flex-wrap items-center justify-center p-4 gap-x-24 gap-y-8 mt-10'>
            {projects.map(({
                id, title, des, img, iconLists, link, imgClass}) => (
             <div key={id} className='sm:h-[41rem] h-[32rem] lg:min-h-[32.5rem] flex items-center justify-center sm:w-[570px] w-[80vw]'>
                <PinContainer title={link} href={link}> 
                    <div className='relative flex items-center justify-center sm:w-[570px] w-[80vw] overflow-hidden sm:h-[40vh]  h-[30vh] mb-10'>
                        ...
                        <img src={img} alt={title} className={`z-10 absolute bottom-0 ${imgClass || 'object-cover'}`} />
                    </div>
                        ...
                        <div className='flex items-center'>
                            {iconLists.map((icon, index) => (
                                <div key={icon} className='border border-white/[0.2] rounded-full bg-black lg:w-10 lg:h-10 w-8 h-8 flex justify-center items-center' style={{transform: `translateX(-${5 * index * 2}px)`}}>
                                    <img src={icon} alt={icon} className={`${icon.endsWith('png') ? '' : 'p-2'}`} />
                                </div>
                                ))}
                        </div>
